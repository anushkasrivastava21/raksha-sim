/*
  ...
  REVISION NOTES -- BUGFIX PASS (post Review 1)
  --------------------------------------------------------
  Three things fixed, everything else byte-for-byte the same as before:

  1. THREAD SAFETY: VitalsRxCallbacks::onWrite() and
     VitalsServerCallbacks::onDisconnect()/onMtuChanged() run on the BLE
     stack's own FreeRTOS task, NOT the Arduino loop() task. The previous
     revision mutated currentState/pendingRequest/subStep directly from
     onWrite() and onDisconnect() with no synchronization against loop() --
     a real cross-task race, invisible in casual testing, that could drop
     or corrupt a request. Fixed with:
       - A tiny single-slot mailbox (cmdBuffer/cmdPending, guarded by a
         portMUX spinlock) handing commands from onWrite() to loop(),
         which is the only place that now touches the state machine.
       - A `bleDisconnectedFlag` so onDisconnect() defers the actual state
         reset to loop() instead of doing it inline on the BLE task.
       - `bleClientConnected` and `negotiatedMtu` marked volatile, since
         they're written on the BLE task and read on the loop() task.
       - A mutex (bleTxMutex) around bleSendRaw(), because PING replies
         are still answered immediately from the BLE task while every
         other packet goes out from loop() -- two tasks could otherwise
         call notify() on the same characteristic concurrently.

  2. SILENT COMMAND DROPS: if a REQ_* came in while busy, the old code
     only logged to Serial -- invisible to the phone, which would wait on
     a notification that was never coming. Now sends a "BUSY|{...}|CRC"
     packet back so the app can show that instead of hanging.

  3. RX CHARACTERISTIC PROPERTIES: was PROPERTY_WRITE only (write-with-
     response). Added PROPERTY_WRITE_NR so the firmware accepts commands
     regardless of whether the phone side uses write-with-response or
...
#include "freertos/semphr.h"  // bleTxMutex -- see thread-safety revision note at top of file
...
// volatile: written on the BLE task (onConnect/onDisconnect/onMtuChanged),
// read on the loop() task (bleSendRaw). See thread-safety revision note.
volatile bool bleClientConnected = false; // replaces btClientConnected
...
// Only ever touched inside bleSendRaw(), which is now mutex-protected, so
// these don't need to be volatile themselves.
uint32_t txDroppedNoClient  = 0;
uint32_t txDroppedOversized = 0;

// ----------------------------------------------------------------------------
// THREAD-SAFETY: BLE task <-> loop() task hand-off
// ----------------------------------------------------------------------------
// onWrite() and onDisconnect() run on the BLE stack's FreeRTOS task, not
// loop(). Rather than mutate the state machine directly from that task,
// onWrite() drops the raw command string into this single-slot mailbox and
// loop() is the only thing that ever calls handleIncomingCommand(). This
// keeps currentState/pendingRequest/subStep/stateEnteredAt single-task-owned
// (loop() only) so none of them need to be volatile or mutex-protected.
static portMUX_TYPE cmdMux = portMUX_INITIALIZER_UNLOCKED;
#define CMD_BUFFER_LEN 24
volatile bool cmdPending = false;
char cmdBuffer[CMD_BUFFER_LEN] = {0}; // only touched inside the cmdMux critical sections below

// Same idea for disconnects: onDisconnect() flags it, loop() does the
// actual state-machine reset.
volatile bool bleDisconnectedFlag = false;

// Guards every call to bleSendRaw(). Needed because PING replies are sent
// straight from the BLE task (for low latency) while every other packet is
// sent from loop() -- without this, the two tasks could call notify() on
// the same characteristic at the same time.
...
class VitalsServerCallbacks: public BLEServerCallbacks {
  ...
  void onDisconnect(BLEServer* server) {
    ...
    // Don't touch currentState/pendingRequest/subStep here -- this runs on
    // the BLE task, and loop() (a different task) owns those. Flag it and
    // let loop() do the actual reset. See thread-safety revision note.
...
class VitalsRxCallbacks: public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* characteristic) {
    ...
    // PING is stateless and latency-sensitive -- answer it immediately
    // from this (BLE) task rather than routing it through loop(). This is
    // the one exception to "only loop() touches the state machine",
    // because PING never touches the state machine at all.
    if (value == "PING") {
      bleSendRaw("PONG");
      return;
    }

    // Everything else (REQ_ECG etc.) gets handed to loop() via a small
    // single-slot mailbox instead of being applied to currentState/
    // pendingRequest here directly -- see thread-safety revision note.
    portENTER_CRITICAL(&cmdMux);
    if (!cmdPending) {
      size_t n = value.length();
      if (n > CMD_BUFFER_LEN - 1) n = CMD_BUFFER_LEN - 1;
      memcpy(cmdBuffer, value.c_str(), n);
      cmdBuffer[n] = '\0';
      cmdPending = true;
    }
    // else: loop() hasn't drained the previous command yet -- drop this
    // one rather than overwrite a not-yet-processed command. In practice
    // this only happens if two commands arrive back-to-back within a
    // single loop() iteration, which shouldn't occur once the system is
    // busy (busy commands are now rejected via the BUSY packet below, not
    // silently ignored -- see handleIncomingCommand()).
...
void setup() {
  ...
  // Guards bleSendRaw() against concurrent calls from the BLE task (PING
  // replies) and the loop() task (everything else). See thread-safety
  // revision note at top of file.
  ...
  pRxCharacteristic = pService->createCharacteristic(
      ...
      // WRITE (with response) + WRITE_NR (without response) -- accept
      // commands from the phone either way, so this doesn't silently fail
      // depending on which write mode Arnav's app happens to use.
...
// ============================================================================
void loop() {
  // Apply a deferred disconnect reset, if the BLE task flagged one since
  // the last iteration. See thread-safety revision note at top of file.
  ...
  // Drain the single-slot command mailbox filled by onWrite() (BLE task).
  // loop() is the only place that ever calls handleIncomingCommand(), so
  // the state machine stays single-task-owned.
  if (cmdPending) {
    char localCmd[CMD_BUFFER_LEN];
    portENTER_CRITICAL(&cmdMux);
    memcpy(localCmd, (const char*)cmdBuffer, CMD_BUFFER_LEN);
    cmdPending = false;
    portEXIT_CRITICAL(&cmdMux);
    handleIncomingCommand(String(localCmd));
  }
...
void handleIncomingCommand(String cmd) {
  ...
  // PING is handled directly in VitalsRxCallbacks::onWrite() now, for
  // lower latency -- it never reaches here. Left unhandled below falls
  // through to "Unknown command" if it somehow does.

  if (currentState != STATE_IDLE) {
    Serial.println(F("[WARN] Request received while busy -- notifying phone"));
    // Previously this was silent on the wire (Serial-only), so a request
    // sent while busy would leave the phone waiting on a notification
    // that was never coming. Now it gets an explicit BUSY packet.
    String json = "{\"requested\":\"" + cmd + "\"}";
    sendPacket("BUSY", json);
    return;
  }
...
// ----------------------------------------------------------------------------
void bleSendRaw(const String &line) {
  // Serializes this function against concurrent callers: PING replies come
  // from the BLE task (VitalsRxCallbacks::onWrite), everything else comes
  // from the loop() task. Without this, both could call notify() on the
  // same characteristic at once. See thread-safety revision note.
  if (bleTxMutex != nullptr) xSemaphoreTake(bleTxMutex, portMAX_DELAY);
  ...
  if (bleTxMutex != nullptr) xSemaphoreGive(bleTxMutex);
}
